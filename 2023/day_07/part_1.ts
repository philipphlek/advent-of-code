...
const CARD_STRENGTH_ORDER = ['A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2']
...
const hasFiveOfAKind = (hand: CardHand): boolean => hand.every(card => card === hand[0])
const hasFourOfAKind = (hand: CardHand): boolean => Object.values(getCardCounts(hand)).includes(4)
const hasFullHouse = (hand: CardHand): boolean => {
  const cardCounts = getCardCounts(hand)
  const counts = Object.values(cardCounts)
  return Math.max(...counts) === 3 && Math.min(...counts) === 2
}
...
const hasTwoPair = (hand: CardHand): boolean => {
  const cardCounts = getCardCounts(hand)
  const counts = Object.values(cardCounts)
  return getPairCount(counts) === 2
}
const hasOnePair = (hand: CardHand): boolean => {
  const cardCounts = getCardCounts(hand)
  const counts = Object.values(cardCounts)
  return getPairCount(counts) === 1
}
