...
function calculateAntinodeCoords(coord1: Coords, coord2: Coords): Coords[] {
  const [rowDiff, colDiff] = getCoordDiff(coord2, coord1)
  const antinodeCoords1 = [coord1[0] + rowDiff, coord1[1] + colDiff] as Coords
  const antinodeCoords2 = [coord2[0] - rowDiff, coord2[1] - colDiff] as Coords
  return [antinodeCoords1, antinodeCoords2]
}
  ...
  Object.values(antennaCoords).forEach((coords) => {
    for (let i = 0; i < coords.length - 1; i++) {
      ...
      for (let j = i + 1; j < coords.length; j++) {
        ...
        const antinodeCoords = calculateAntinodeCoords(coord1, coord2)
        antinodeCoords.forEach((antinodeCoord) => {
          if (isCoordsValid(antinodeCoord, grid)) {
